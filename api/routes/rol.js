...
router.get('/todos', (req, res, next) => {
    ...
    pool.connect()
    .then(client => {
        return client.query("select * from sel_rol_fil('') as (id integer, nombre varchar(30), createdat timestamp without time zone, updatedat timestamp without time zone)")
        .then(result => {
            client.release()
            results = result.rows;
